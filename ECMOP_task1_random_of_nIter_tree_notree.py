...
from mol_utils import get_fragments, mol_ec_random_tree_notree, mol_grade, pre_mol_grade, select_pop, child_mol_ec_random_tree_notree,child_mol_ec_random_of_nIter_tree_notree, pre_mol_grade_plogP, \
...
def main(fragment_file, lead_file):
    ...
    df_to_save_A_to_B.to_csv('./test/task1/p1~2_ep60_nIter50_cq0_0.85_0_random1_of_nIter.csv', index=False)
...
def ec(lead_mols,X, decodings, p, ec_epoch, tree_sum, num, nIter, child_ec_epoch, successs, results, index, total, lock,isfmpo,chongqi,random):
    ...
    while i<end:
        ...
        for iter in range(nIter):
            pops = child_mol_ec_random_of_nIter_tree_notree(pops, p, ec_epoch + 1, child_ec_epoch, tree_sum,iter+1,nIter)
        ...
        if resplash[i]==0:
            ...
            all_chongqi_fits = mol_grade_task1_sim(all_chongqi_pops,lead_mols[i],decodings,isfmpo)
        ...
        if resplash[i]==chongqi:
            ...
            for j in range(pops.shape[0]):
                ...
                if child_plogp - pre_plogp >= 3 and child_qed>=0.85 and child_sim >=0.3:
                    ...
                    judge = False
                print(f'第{i+1}个分子子代：qed:{child_qed},plogp_imp:{round(child_plogp - pre_plogp, 3)},sim:{child_sim},judge:{judge}')
